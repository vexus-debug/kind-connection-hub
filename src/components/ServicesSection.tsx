        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12 animate-fade-in">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
              <span className="bg-gradient-to-r from-primary-deep via-primary to-accent-green bg-clip-text text-transparent" style={{ textShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
                Luxury Services, Day & Night
              </span>
            </h2>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              From therapeutic massages and organic facials to Jacuzzi baths and organic scrubs,
              experience world-standard treatments with 100% organic products, any time of day.
            </p>
          </div>
          ...
          <div className="mb-16 animate-fade-in">
            <div className="relative rounded-2xl overflow-hidden shadow-lg group">
              <img 
                src={servicesImage} 
                alt="Faridaz Spa world-class wellness treatments and luxury services" 
                loading="lazy"
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <h3 className="text-xl md:text-2xl font-bold text-white mb-2">
                  World-Class Spa and Health Club, 24/7
                </h3>
                <p className="text-white/90 text-sm md:text-base">
                  100% organic products • Jacuzzi • Sauna • Professional therapists
                </p>
              </div>
            </div>
          </div>
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {servicesData.map((category, index) => (
                <AccordionItem 
                  key={index} 
                  value={`item-${index}`}
                  className="bg-card border border-border/80 rounded-xl overflow-hidden hover:border-primary/50 hover:shadow-elegant transition-all duration-200"
                >
                  <AccordionTrigger className="px-5 py-4 hover:no-underline group">
                    <div className="flex items-center gap-3 w-full">
                      <div className="w-10 h-10 bg-gradient-to-br from-primary to-gold rounded-lg flex items-center justify-center text-white group-hover:scale-105 transition-transform duration-200">
                      ...
                      {category.badge && (
                        <span className={category.badge === "Featured" ? "badge-featured" : "badge-popular"}>
                  ...
                  <AccordionContent className="px-5 pb-5">
                    <div className="space-y-2 pt-2">
                      {category.services.map((service, serviceIndex) => (
                        <div 
                          key={serviceIndex}
                          className="flex items-start justify-between gap-3 p-2 rounded-lg hover:bg-secondary/30 transition-colors duration-200 group/item"
                        >
                          <div className="flex flex-col gap-1 flex-1">
                            <div className="flex items-start gap-2">
                              ...
                              <span className="text-sm text-foreground/80 leading-relaxed group-hover/item:text-primary transition-colors">
                                {service.name}
                              </span>
                            </div>
                            {(service.duration || service.note) && (
                              <div className="ml-3.5 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                {service.duration && (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/5 rounded-full">
                                    ⏱️ {service.duration}
                                  </span>
                                )}
                                {service.note && (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gold/10 rounded-full text-gold">
                                    📍 {service.note}
                                  </span>
                                )}
                              </div>
                    ...
                    <button
                      ...
                      className="mt-4 w-full flex items-center justify-center gap-2 text-sm font-medium text-gold hover:text-primary transition-colors duration-200 group/book"
                    >
          ...
          <div className="mt-16 text-center animate-fade-in">
            <div className="bg-gradient-to-br from-primary/5 to-gold/5 rounded-2xl p-8 md:p-12 border border-primary/10">
              <div className="inline-flex items-center gap-2 mb-4 px-3 py-1 bg-primary/10 rounded-full">
                <Sparkles className="w-4 h-4 text-primary" />
                <span className="text-xs font-semibold text-primary">Limited Slots Available</span>
              </div>
              
              <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">
                <span className="bg-gradient-to-r from-primary to-gold bg-clip-text text-transparent">
                  Ready for Your Spa and Health Club Journey?
                </span>
              </h3>
              
              <p className="text-base md:text-lg text-muted-foreground mb-6 max-w-2xl mx-auto">
                Book your appointment today and discover why Faridaz Spa is Abuja's 
                premier destination for luxury wellness treatments.
              </p>
              
              <div className="mb-6 p-4 bg-gold/5 border border-gold/20 rounded-xl">
                <p className="text-sm text-muted-foreground">
                  💡 <strong className="text-foreground">Note:</strong> Extra time attracts extra charges.
                </p>
              </div>
